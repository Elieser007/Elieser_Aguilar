<template>
    <div class="relative text-gray-500 focus-within:text-purple-600">
        <select ref="input"
            ...
            <option v-if="!modelValue" value="" disabled selected>{{ text }}</option>
            <option v-for="op in options" :key="op[optIdField] || op.id" :value="op[optIdField] || op.id">
                {{ op[optTextField] || op.text }}
            </option>
        </select>
...
defineProps(['modelValue', 'text', 'options', 'optIdField', 'optTextField', 'required']);
